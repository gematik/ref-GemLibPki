...
import de.gematik.pki.gemlibpki.utils.*;
...
class TslSignerTest {
  ...
  @BeforeAll
  public static void setup() throws XAdES4jException {
    tslRsa =
        TslReader.getTslAsDoc(ResourceReader.getFilePathFromResources(TSL_PATH_RSA)).orElseThrow();
    trustAnchorRsa =
        CertReader.readX509(
            Utils.readContent(ResourceReader.getFilePathFromResources(TRUSTANCHOR_PATH_RSA)));
    final P12Container signerRsa = readSignerCert(SIGNER_PATH_RSA);
    TslSigner.sign(tslRsa, signerRsa);
    tslRsaNoSig =
        TslReader.getTslAsDoc(ResourceReader.getFilePathFromResources(TSL_PATH_RSA_NOSIG))
            .orElseThrow();
    TslSigner.sign(tslRsaNoSig, signerRsa);
    tslEcc =
        TslReader.getTslAsDoc(ResourceReader.getFilePathFromResources(FILE_NAME_TSL_DEFAULT))
            .orElseThrow();
    trustAnchorEcc =
        CertReader.readX509(
            Utils.readContent(ResourceReader.getFilePathFromResources(TRUSTANCHOR_PATH_ECC)));
  ...
  void verifySignatureWrongTa() {
    ...
    final Document tslAltTa =
        TslReader.getTslAsDoc(ResourceReader.getFilePathFromResources(FILE_NAME_TSL_DEFAULT_ALT_TA))
            .orElseThrow();
