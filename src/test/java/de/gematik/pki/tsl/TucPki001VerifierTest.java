...
class TucPki001VerifierTest {
    ...
    @BeforeAll
    static void start() {
        tspServicesInTruststore = new TslInformationProvider(
            TslReader.getTsl(ResourceReader.getFilePathFromResources(FILE_NAME_TSL_DEFAULT)).orElseThrow()).getTspServices();
        newTslToCheck = TslReader.getTsl(ResourceReader.getFilePathFromResources(FILE_NAME_TSL_DEFAULT)).orElseThrow();
    }
    ...
    void verifyPerformTucPki001ChecksValid() {
        ...
        final X509Certificate tslSigner = P12Reader.getContentFromP12(
            Path.of("src/test/resources/certificates/GEM.TSL-CA8/TSL-Signing-Unit-8-TEST-ONLY.p12"), "00").getCertificate();
        ...
        final TucPki001Verifier tucPki001Verifier = TucPki001Verifier.builder()
    ...
    void verifyNullChecks() {
        final var builder = TucPki001Verifier.builder();
