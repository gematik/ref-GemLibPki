...
public class TucPki018Verifier {
  ...
  public Admission performTucPki18Checks(@NonNull final X509Certificate x509EeCert)
    ...
    final TspServiceSubset tspServiceSubset =
        new TspInformationProvider(tspServiceList, productType).getTspServiceSubset(x509EeCert);
  ...
  protected void doOcspIfConfigured(
      final X509Certificate x509EeCert, final TspServiceSubset tspServiceSubset)
      throws GemPkiException {
    if (withOcspCheck) {
  ...
  /**
   * Verify given end-entity certificate against the list of parameterized certificate profiles
