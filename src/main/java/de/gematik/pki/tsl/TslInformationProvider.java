...
public class TslInformationProvider {
    ...
    public List<TspService> getFilteredTspServices(final List<String> stiFilterList) {
        ...
        return trustServiceStatusList.getTrustServiceProviderList().getTrustServiceProvider()
            .stream()
    ...
    public List<TspService> getTspServicesForTsp(final String tsp, final List<String> stiFilterList) {
        final List<TSPType> tspTypes =
            trustServiceStatusList.getTrustServiceProviderList().getTrustServiceProvider()
                .stream()
                .filter(c -> tsp.contains(c.getTSPInformation().getTSPName().getName().get(0).getValue()))
                .collect(Collectors.toList());
